...
static void ARAMClear(void)
{
  ...
  for (i = AR_GetBaseAddress(); i < AR_GetSize(); i += 2048)
  {
...
int DOLtoARAM(unsigned char *dol)
{
  ...
  /*** Make sure ARAM subsystem is clear! ***/
  ARAMClear();
  ...
  for (i = 0; i < MAXTEXTSECTION; i++)
  {
    ...
    {
      ARAMPut(dol + dolhdr->textOffset[i], (char *) ((dolhdr->textAddress[i] - minaddress) + AR_GetBaseAddress()),
              dolhdr->textLength[i]);
    }
  }
  ...
  for (i = 0; i < MAXDATASECTION; i++)
  {
    ...
    {
      ARAMPut(dol + dolhdr->dataOffset[i], (char *) ((dolhdr->dataAddress[i] - minaddress) + AR_GetBaseAddress()),
              dolhdr->dataLength[i]);
    }
  }

  /*** Now go run it ***/
  ARAMRun(dolhdr->entryPoint, minaddress, AR_GetBaseAddress(), sizeinbytes);
